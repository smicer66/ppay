        ...
        <div class="box">
            ...
            <div class="box-body">
                <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
                    ...
                    <div class="box-header">
                        <h3 class="box-title">Accounts Attached</h3>
                    </div>

                    @if(isset($wallet) && $wallet!=NULL)
                        <div class="row">
                            <div class="col-sm-12">
                                <label for="inputEmail3" class="col-sm-2 control-label">Wallet Code:</label>
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                ...
                                <div class="col-sm-10">
                                    {{$wallet->customer->lastName}}, {{$wallet->customer->firstName}} {{$wallet->customer->otherName}}
                                </div>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
                                <thead>
                                    <tr role="row">
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="example2"
                                            rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Account No</th>
                                        <th class="sorting" tabindex="0" aria-controls="example2"
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1"
                                            aria-label="CSS grade: activate to sort column ascending">Bank</th>
                                        <th class="sorting" tabindex="0" aria-controls="example2"
                                            rowspan="1" colspan="1"
                                ...
                                ?>
                                <tbody>
                                    @foreach($walletAcctAttachedList as $walletAccount)
                                        <?php
                                          $id = \Crypt::encrypt($walletAccount->id);
                                        ?>
                                    <tr role="row" class="odd">
                                        <td class="sorting_1"><input type="checkbox" name="accounts[]"></td>
                                        <td class="sorting_1">{{$walletAccount->account->accountIdentifier}}</td>
                                        <td>{{$walletAccount->account->accountType}}</td>
                                        <td class="sorting_1">{{$walletAccount->account->branchCode}}</td>
                                        <td>{{$walletAccount->account->bank->bankName}}</td>
                                        <td>{{$walletAccount->account->status}}</td>
                                        <td>
                                            <div class="btn-group">
                                                ...
                                                <ul role="menu" class="dropdown-menu">
                                                    @if(\Auth::user()->role_code == \App\Models\Roles::$POTZR_STAFF)
                                                        @if(isset($walletAccount->deleted_at) && $walletAccount->deleted_at!=NULL)
                                                            <li><a href="/potzr-staff/e-wallet/reactivate-account/{{$id}}">Re-Add Account to Wallet</a></li>
                                                        @else
                                                            <li><a href="/potzr-staff/e-wallet/suspend-account/{{$id}}">Remove Account to Wallet</a></li>
                                                        @endif
                                                    @endif
                                                </ul>
                                            </div>
