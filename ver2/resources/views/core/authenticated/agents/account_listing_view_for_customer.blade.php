...
@if(isset($all_accounts) && $all_accounts!=null && sizeof($all_accounts)>0)

@else
<div aria-hidden="true" class="onboarding-modal modal fade animated show-on-load" role="dialog" tabindex="-1" id="wallet_overview_modal">
        ...
        <div class="modal-content text-center">
            ...
            <div style="padding: 60px 70px;">
                @if(isset($wallet) && $wallet!=null)
                    <h2 style="margin-top: 0px;" id="walletOverviewBalance">
                        ZMW0.00
                    </h2>
                ...
                @else
                    ...
                    <div style="color: #636363; font-size: 14px;">
                        Hello {{\Auth::user()->firstName}}, you need a wallet to hold funds you intend to spend. After opening a wallet, you can subsequently order a physical card or setup a virtual card to manage the funds
                    </div>
                    <a onclick="newWallet('{{\Session::get('jwt_token')}}')" data-target="#cards_overview_modal" data-toggle="modal" style="cursor: pointer !important; padding: 5px 15px; background-color: #4B72FA; color: #fff; font-weight: bolder; font-size: 14px; display: inline-block; margin: 20px 0px; margin-right: 20px; text-decoration: none;">Setup A Wallet For Me</a>
                @endif
